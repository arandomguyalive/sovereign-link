    ...
    <group ref={ref} position={data.pos} onClick={(e) => { e.stopPropagation(); onSelect(data); }}>
      ...
      <Html distanceFactor={60} position={[0, 5, 0]} center>
        <div className="flex flex-col items-center gap-1 cursor-pointer group">
          <div className="p-1.5 bg-black/90 border border-white/20 rounded-full group-hover:border-neon-cyan shadow-xl" style={{ color: getColor() }}><Icon /></div>
          <div className="text-[7px] text-white font-black bg-black/95 px-2 border border-white/10 whitespace-nowrap uppercase tracking-widest">{data.type === 'AGENT' ? data.details.heart : data.type === 'VEHICLE' ? data.details.speed : data.id}</div>
        </div>
      </Html>
    </group>
  );
  ...
  return (
    <div className="w-full h-full bg-black relative cursor-crosshair overflow-hidden pointer-events-auto select-none touch-none">
      ...
      <div className="absolute top-32 left-4 w-60 bottom-32 hidden xl:flex flex-col gap-3 pointer-events-none">
        ...
        <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pr-4">{entities.slice(0, 30).map((e, i) => (<div key={i} className="bg-black/60 backdrop-blur-sm border-l-2 border-white/10 p-2 animate-in slide-in-from-left duration-500" style={{ animationDelay: `${i * 40}ms` }}><div className="flex justify-between"><div className="text-[9px] text-white/80 font-black">{e.id}</div><div className="text-[7px] text-emerald-400/60 font-mono">TRACKING</div></div><div className="text-[7px] text-neon-cyan/60 font-mono mt-1 uppercase italic">Lat: {e.details.lat} Lng: {e.details.lng}</div></div>))}</div>
      </div>
